...
export default async function execute<T>(
	...
	try {
		...
		if (axios.isAxiosError(err)) {
			...
			if (response) {
				debugLogger.debug(`[${funcTitle}] handled via onAxiosException`);
				return response;
			}

			const message =
				getNetworkErrorMessage(err) ?? getServiceErrorMessage(err) ?? getHttpStatusErrorMessage(err) ?? errorMessage;

			debugLogger.error(`[${funcTitle}] AxiosError:`, message);
		...
		if (err instanceof Error) {
			...
			if (err.message.includes("CERT_") || err.message.includes("SSL_")) {
				debugLogger.error(`[${funcTitle}] SSL/TLS error:`, err);
		...
		debugLogger.error(`[${funcTitle}] Unexpected error:`, err);
		if (takeScreenshotError) {
			debugLogger.error(`[${funcTitle}] Error screenshot:`, err);
		}
