...
class Repository {
  ...
  private _disposers = new Map<
    string,
    (disposeAble: DisposeAble) => void | Promise<void>
  >();
    ...
    if (Repository._isInitAble(repository)) {
      ...
      if (initResult instanceof Promise) {
        initResult.catch((error) => {
          console.error(
            `[Repository] Initialization failed for ${key}:`,
            error
          );
        });
      }
  ...
  static get<T extends object>(instanceName?: string): T {
    if (!instanceName) {
      throw new Error(
        '[Repository] instanceName is required. Please provide instanceName.'
      );
    }
    ...
    if (!key) {
      throw new Error(
        `[Repository] Service with instanceName "${instanceName}" is not registered.`
      );
    }
    ...
    if (factory) {
      ...
      if (
        Repository._isDisposeAble(service) &&
        !repoInstance._disposers.has(key)
      ) {
   ...
   */
  private _addInstanceNameFromTypeName(
    typeName: string,
    instanceName: string
  ): void {
   ...
   */
  private static _getKeyFromTypeName(
    typeName: string,
    instanceName: string
  ): string {
  ...
  private static _isDisposeAble(obj: unknown): obj is DisposeAble {
    return (
      ...
      obj !== null &&
      'dispose' in obj &&
      typeof (obj as DisposeAble).dispose === 'function'
    );
  }
}
...
function repositoryRegister(): void {
  ...
  Repository.register<DashboardRepository>(
    new DashboardRepositoryImpl(
      new DashboardApiImpl(),
      'dashboard:selectedFilter:income-pos'
    ),
  ...
  Repository.register<DashboardRepository>(
    new DashboardRepositoryImpl(
      new DashboardApiImpl(),
      'dashboard:selectedFilter:income-accounting'
    ),
    { instanceName: 'Dashboard-Income-Accounting' }
  );

  // Daily Income POS
  Repository.register<DailyIncomePosRepository>(
    new DailyIncomePosRepositoryImpl(new DailyIncomePosApiImpl()),
    {
  ...
  // Customer Info
  Repository.register<CustomerInfoRepository>(
    new CustomerInfoRepositoryImpl(new DashboardApiImpl()),
    {
  ...
  // Vendor Info
  Repository.register<VendorInfoRepository>(
    new VendorInfoRepositoryImpl(new DashboardApiImpl()),
    {
  ...
  // Performance
  Repository.register<PerformanceRepository>(
    new PerformanceRepositoryImpl(new DashboardApiImpl()),
    {
