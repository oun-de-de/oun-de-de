	...
	return (
		<>
			{Array.from({ length: 3 }).map((_, idx) => (
				<StyledPerformanceItem key={idx} className="flex items-center justify-between bg-white">
...
export default function PerformanceCard({ item }: { item: PerformanceItem }) {
	return (
		<StyledPerformanceItem key={item.id} className="flex items-center justify-between bg-white">
			<div className="flex items-center gap-3">
				<Badge variant={item.variant} shape="dot" className="h-6 w-1 p-0 border-none shadow-none" aria-hidden="true" />
				<StyledPerformanceItemLabel>{item.label}</StyledPerformanceItemLabel>
			</div>
			<Badge variant={item.variant} className="rounded-md px-2">
				<StyledPerformanceItemValue>{formatKHR(item.value)}</StyledPerformanceItemValue>
			</Badge>
		</StyledPerformanceItem>
	);
}
